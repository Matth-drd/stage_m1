...
Poisson -- des approches ensemblistes -- Random Forest, Gradient Boosting -- enfin, une approche 
...
la log-loss et le ROI, que nous avons détaillés dans le chapitre \ref{chap:métriques}.


\section{Modèles statistiques}
\label{sec:stat_poisson}
...
manière statique sur l'ensemble de la saison. Cette approche introduit une 
fuite de données majeure, car des informations chronologiquement futures sont 
...
leur sens statistique d'espérances de buts par match, tout en 
...
Dans l'introduction, nous avons abordé les améliorations apportées par Dixon-Coles. 
Nous les détaillons à présent plus en profondeur, en débutant par le facteur 
qui gère la dépendance des faibles scores.
\[
\tau(x, y) = 
\begin{cases} 
...
\noindent Nous avons ainsi, 
$$L(\alpha, \beta, \gamma, \delta, \rho) = \tau(x, y, \mu, \nu, \rho) \times 
...
selon leur ancienneté. Pour cela, ils ajoutent une fonction $\phi$ définie par : 
$$\phi_k = e^{-\xi \dot t_k}$$ 
cette fonction permet de donner plus d'importance aux matchs récents. 
...
le nombre de buts, l'autre sur l'issue du match. Les modèles statistiques implémentés se basent sur le nombre de buts. 
...
Pour cela, selon Maher, les buts marqués par l'équipe A et par l'équipe B sont indépendants. 
Par suite, la probabilité $P(X=x, Y=y) = P(X=x) \times P(Y=y)$ ce qui nous permet de 
construire la matrice $M$ telle que $M_{i,j} = P(X=i, Y=j)$ figure \ref{fig:matricMaher}. 
Ainsi, la probabilité que l'équipe A gagne correspond à la probabilité qu'elle 
marque plus de buts que l'équipe B. Autrement dit, la victoire de l'équipe à domicile 
correspond aux coefficients de la matrice tels que $i>j$. Le match nul correspond à 
la diagonale, et la victoire extérieure correspond aux coefficients de la matrice triangulaire 
...
\begin{figure}[H]
    ...
    \begin{minipage}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../../plot/matrix_maher.png}
    ...
    \begin{minipage}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../../plot/matrix_DC.png}
...
\begin{figure}[H]
    \centering
    % --- Image du Haut ---
    \includegraphics[width=0.85\textwidth]{../../plot/decomp_maher.png}
    ...
    % --- Image du Bas ---
    \includegraphics[width=0.85\textwidth]{../../plot/decomp_DC.png}
    \caption{Modèle de Dixon-Coles : Séparation en 3 matrices distinctes}
    \label{fig:matrixDCSeparation}
\end{figure}

Dans le cas de Dixon \& Coles, l'hypothèse d'indépendance n'est pas tout à fait la même 
concernant les faibles scores. 
Pour traiter ce cas, ils ajoutent un facteur $\tau$, la matrice $M$ devient donc : 
$M_{i,j}=\tau(i,j)\times P(X=i)\times P(Y=j)$. L'interprétation reste similaire à 
celle de Maher, voir les figures \ref{fig:matricDC} et \ref{fig:matrixDCSeparation}.
...
au même titre que les autres algorithmes. Nous comparerons également une 
version de base et optimisée.

\subsection{LightGBM}

Nous avons également implémenté un LightGBM \cite{chenlgbm}. 
...
\begin{figure}[H]
    ...
    \centering
    \caption{Level-wise tree growth. \\Extrait de la documentation de LightGBM. \\
...
précédentes par l'utilisation d'un espace de caractéristiques dédié (\texttt{ft\_nn}), potentiellement 
...
($\texttt{tol} = 10^{-5}$). Une version optimisée est ensuite obtenue par Random Search.
