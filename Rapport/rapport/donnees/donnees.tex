...
Les données ont été extraites du portail \texttt{football-data.co.uk}, couvrant
...
\subsection{Distribution des résultats}
...
Cette distribution confirme l'avantage du terrain dans le football.
...
L'étape de préparation a été réalisée sous Python avec la bibliothèque
...
Le rapport tirs cadrés / tirs tentés mesure la qualité offensive. Une variante
...
\begin{longtable}{lllp{5.5cm}}
    ...
    \midrule
    \endhead
    Forme (FT/HT)     & \texttt{XX\_Hforme}       & \texttt{XX\_Aforme}       & Moyenne buts (5 derniers matchs) \\
    ...
    Elo               & \texttt{XX\_Elo\_H}        & \texttt{XX\_Elo\_A}        & Niveau de puissance avant match \\
    ...
    Efficacité pond.  & \texttt{FT\_Hprec\_weight} & \texttt{FT\_Aprec\_weight} & Précision pondérée ($w_g = 1{,}5$) \\
...
entre eux à l'aide d'un Critical Difference diagramme. Cette méthode se 
base sur un test de Friedman afin de déterminer s'il éxiste ou non des différences 
...
Ce découpage permet d'entraîner un classifieur binaire spécialisé pour
chaque issue, cela permet également de rééquilibrer les classes,
nous passons d'une distribution 45\% - 25\% - 30\% à 45\% - 55\%.
...
\begin{itemize}
 ...
 acteurs. Le rejet de l'hypothèse nulle ($p < 0,05$) indique
 ...
 d'acteurs dont les performances diffèrent significativement.
 ...
 les groupes d'acteurs dont la différence de performance n'est pas
 statistiquement significative au seuil $\alpha = 0,05$.
\end{itemize}

Cette méthodologie permet de situer précisément notre modèle MLP au sein de
la hiérarchie du marché, en distinguant les gains de performance réels des
fluctuations aléatoires des données.
