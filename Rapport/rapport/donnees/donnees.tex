...
\noindent L'étape de préparation consiste à transformer les données brutes issues des
dix saisons et cinq championnats en un dataset exploitable pour l'apprentissage 
évitant la fuite de données.
...
cotes des bookmakers (Bet365, Pinnacle, etc) cette logique est abordée dans 
\cite{rogier}. La méthodologie employée est la suivante, 
dans un premier temps nous inversons les cotes afin de convertir les cotes décimales en 
...
bookmaker soit égale à 1, cela permet de supprimer la marge des bookmakers.

Cette étape garantit que le modèle et les bookmakers sont évalués sur le même 
espace probabiliste : $P(Home) + P(Reste) = 1$.

Par la suite, nous avons procédé à la comparaison des bookmakers à l'aide d'un diagramme des 
différences critiques (Critical Difference Diagram)
\footnote{https://mirkobunse.github.io/CriticalDifferenceDiagrams.jl/dev/}. 
Cette méthode repose d'abord sur le test de Friedman, qui permet de déterminer si 
des différences statistiquement significatives existent entre les bookmakers 
considérés. Lorsque ce test met en évidence une différence globale significative, 
une analyse post hoc basée sur un test de Nemenyi peut alors être 
réalisée afin d'identifier les bookmakers qui 
se distinguent significativement les uns des autres. Le diagramme représente les 
rangs moyens obtenus par chaque bookmaker et met en évidence les groupes dont les 
performances ne diffèrent pas significativement. Ces deux tests sont détaillés 
dans \cite{demsar}.
...
\begin{figure}[H]
    ...
    \label{fig:cdd}
\end{figure}
\noindent Sur la figure \ref{fig:cdd} la crossbar noire reliant deux couleur indique qu'il n'y a pas 
